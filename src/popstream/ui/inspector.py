...
class PropertyInspector(QWidget):
    def __init__(self, engine: Engine, parent=None) -> None:
        ...
        self.plugin_layout.setContentsMargins(0, 8, 0, 0)
    ...
    def refresh(self) -> None:
        ...
        if slot is None:
            ...
            self.remove.hide()
            self.test.hide()
            self.icon.setPixmap(icon_pixmap("grid", 48, C.muted))
            self._loading = False
            return
        ...
        self.icon.setPixmap(icon_pixmap(info.icon if info else "grid", 56))
